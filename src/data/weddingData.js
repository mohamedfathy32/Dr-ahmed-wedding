export const weddingData = {
  groom: {
    name: 'د. أحمد محمود',
    fullName: 'الدكتور أحمد محمود',
    title: 'طبيب',
  },
  displayName: 'د. أحمد محمود',
  welcomeMessage: 'يتشرف الدكتور أحمد محمود بدعوتكم لحضور حفل زفافه',
  story:
    'بداية رحلة جديدة مليئة بالمحبة والسعادة. نتمنى أن تكونوا جزءًا من أجمل لحظات حياتنا ونشارككم فرحتنا في هذا اليوم المميز.',
  weddingDate: new Date('2026-06-27T20:00:00'),
  eveningParty: {
    date: 'الجمعة، ٢٦ يونيو ٢٠٢٦',
    location: 'أمام المنزل',
  },
  details: {
    date: 'السبت، ٢٧ يونيو ٢٠٢٦',
    time: '٨:٠٠ مساءً',
    venue: 'قاعة جاردينا',
    address: 'آخر طريق البلاجات، الإسماعيلية',
  },
  location: {
    mapUrl: 'https://maps.app.goo.gl/KnPFxiNJA1bGZWrR6?g_st=iw',
    label: 'عرض الموقع على الخريطة',
  },
  schedule: [
    { time: 'الجمعة ٢٦ يونيو', event: 'السهرة - أمام المنزل' },
    { time: '٧:٣٠ مساءً', event: 'استقبال الضيوف - قاعة جاردينا' },
    { time: '٨:٠٠ مساءً', event: 'بداية الحفل' },
    { time: '٩:٠٠ مساءً', event: 'العشاء' },
    { time: '١٠:٠٠ مساءً', event: 'التصوير' },
  ],
  gallery: [
    {
      src: 'https://images.unsplash.com/photo-1519741497674-611481863552?w=800&q=80',
      alt: 'صورة تذكارية',
    },
    {
      src: 'https://images.unsplash.com/photo-1606216794074-735e91aa2c92?w=800&q=80',
      alt: 'صورة العريس',
    },
    {
      src: 'https://images.unsplash.com/photo-1465495976277-4387d4b0b4c6?w=800&q=80',
      alt: 'لحظات مميزة',
    },
    {
      src: 'https://images.unsplash.com/photo-1511285560929-80b456fea0bc?w=800&q=80',
      alt: 'ذكريات جميلة',
    },
    {
      src: 'https://images.unsplash.com/photo-1520854221256-17451b1c6296?w=800&q=80',
      alt: 'احتفال الزفاف',
    },
  ],
  heroImage:
    'https://images.unsplash.com/photo-1519741497674-611481863552?w=1920&q=80',
  heroVideo:
    'https://www.pexels.com/download/video/27979648/',
  heroVideoFallback:
    'https://www.pexels.com/download/video/27979648/',
  storyImage:
    'https://images.unsplash.com/photo-1606216794074-735e91aa2c92?w=800&q=80',
  footerMessage: 'نتشرف بحضوركم ومشاركتكم فرحتنا',
  envelopeMessage: 'هذه الدعوة خاصة بك',
  sealInitials: 'A & H',
};
